					...
					pub: function (targetElement, opts) {
						...
						this.trigger = function (eventNames) {
							...
							for (var i = 0; i < evs.length; i += 1) {
								...
								let valueHex = this.valueElement.getAttribute('data-current-color')
								let rgba = hexToRgb(valueHex.substring(0, 7))

								
								async function toServer(params) {
									//if (!boolFlagDelivery) return;
									//boolFlagDelivery = false;
									let response = await fetch(window.location.href+'rgb', {
										method: 'POST', 
										headers: {
...
//.then((value)=>boolFlagDelivery = true)
								}
								toServer(rgba)
								console.log(hexToRgb(valueHex.substring(0, 7)))
